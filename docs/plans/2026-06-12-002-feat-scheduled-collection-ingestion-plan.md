...
- **U1 spike findings (executed 2026-06-15 — PASS; headless agentic ingest is feasible, build U2–U7):**
  - **(a) Binary + env.** `/Users/jonasblasques/.claude/local/claude` (a bash wrapper → `~/.claude/local/node_modules/.bin/claude`), v2.1.177. Runs headless from a minimal/sandboxed shell; needs network + the local OAuth tokens (no `ANTHROPIC_API_KEY` required for OAuth auth). `claude` is NOT on PATH → the orchestrator must use this absolute path (KTD 4).
  - **(b) Unattended permission mode.** `--print --permission-mode bypassPermissions` together with a scoped `--allowedTools "Read" "Write" "Edit" "Glob" "Grep" "LS"` performed all file writes with **`permission_denials: []`** — no prompts, no hangs. (`--dangerously-skip-permissions` is the blunter alternative.) Recommend `bypassPermissions` + an explicit allow-list over skip-all.
  - **(c) Output capture.** `--output-format json` returns a single JSON object; capture `result` (final text summary), `is_error`, `permission_denials` (assert empty), `num_turns`, `duration_ms`, `total_cost_usd`, `terminal_reason`. Clean to parse for the U3 run report.
  - **(d) Gate-honoring (the central risk) — VERIFIED on disk, not just self-reported.** Seeded a 2-source sample: one fitting an existing domain (dbt → `data-engineering`), one fitting none (bonsai horticulture = a new-domain decision). With a safe-subset `INGEST.md` instructing "route to existing domains only; defer any new-domain/collision/contradiction/20+-cascade to `_REVIEW.md` and skip," headless claude **wrote the dbt page and deferred the bonsai source to `_REVIEW.md`, creating no unauthorized domain or page.** Filesystem-verified (no `corpus/bonsai/`, deferral line present).
  - **(e) Timing/cost.** Fixed overhead ≈ 5 s + ~$0.30 per invocation (fresh session loads system prompt/tools). The 2-source gated ingest took ~30 s wall-clock, 9 turns, ~$0.47 → roughly **~13 s and ~$0.10–0.20 per small source** on top of the fixed overhead. For U3 defaults: budget a generous per-source timeout (~60 s/source) and a wall-clock `timeout` sized to `max_n`; expect real (longer) sources to cost more.
  - **Conclusion:** the fallback (collect-only + notify) is NOT needed; proceed to build U2–U7 on these mechanics. Exact invocation string for U3 `run_ingest`:
    `</Users/jonasblasques/.claude/local/claude> --print "<ingest prompt / point at the /ingest-auto skill>" --output-format json --permission-mode bypassPermissions --allowedTools Read Write Edit Glob Grep LS` (run with `cwd` = repo root; parse `.result` / `.is_error` / `.permission_denials`).
