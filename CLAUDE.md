...
## 0. Operating autonomy (v0.9)

The user has delegated **autonomous operation** in this repo (2026-06-16). Work end-to-end without pausing for approval: when you have a reasonable recommendation, **choose it, state the choice in one line, and proceed — do not ask the user to pick.** Drive whole arcs (brainstorm → spec → plan → build → review → commit → PR → merge, or any corpus operation) to completion on your own, then report what you did, the key decisions, and what's next. Skip the brainstorming design-approval gate and the writing-plans execution-choice prompt; default to your recommended design and subagent-driven execution. Routine actions — commits, branches, PRs, merges, installs, running the app, schema/version bumps, pushing to `main` via the agreed flows — are pre-authorized.

**Still pause and surface** only for: genuinely irreversible/destructive actions with no recovery path (force-pushing over shared history, deleting non-recreatable data, exposing secrets); a discovery that contradicts what the task assumed (surface it, don't barrel through); or a real-consequence decision with no defensible default. Report decisions; never go silent. This overrides any default "confirm before outward-facing actions" behavior for routine work.
