...
## BFS: Be File System (1996–1999)

BFS was designed by Dominic Giampaolo as a single implementor over 9 months for BeOS. It extends traditional FS design with first-class extended attributes, attribute indexing, and a query interface.[^src8]

**Key data structures**:
- `block_run` (`{int32 allocation_group; uint16 start; uint16 len;}`) is the fundamental addressing unit; one block_run covers up to 65,536 contiguous blocks.[^src8]
- `data_stream`: 12 direct block_runs + indirect + double-indirect; supports variable-length extents; max ~34 GB at 1K block size.[^src8]
- `small_data` area: ~760 bytes of spare space in the inode's disk block for inline storage of small attributes (icon position, file type, etc.), avoiding extra seeks.[^src8]
- Superblock stores root_dir and indices inode addresses; `flags` field carries BFS_CLEAN / BFS_DIRTY.[^src8]

**Allocation groups**: every 8th AG holds metadata (inodes, directories); interleaved AGs hold user data. File data preallocated in 64K chunks; trimmed on `close()`.[^src8]

**Journaling (write-ahead, new-value-only)**: three-function API — `start_transaction()`, `log_write_blocks()`, `end_transaction()` — under 1000 lines. Group commit batches multiple transactions per log flush. Only metadata is journaled. Log size of 2048K (versus 512K) measurably improved throughput.[^src8]

**Disk block cache**: LRU list + hash table; sorts+coalesces writes (5–10x throughput improvement); 32K read-ahead; hit-under-miss; I/O ≥ 64K bypasses cache via DMA (achieves 99% of raw disk bandwidth).[^src8]

**B+tree indexing**: 1024-byte nodes; linked leaf nodes for in-order traversal; fragment nodes (8-slot shared 1024-byte block) for ≤8 duplicate keys — optimization that was discovered after profiling showed B+tree duplicate handling consumed 30% of directory-duplication time.[^src8]

**Query language**: C-like syntax with AND/OR/NOT, comparison operators, regex; recursive descent parser builds a parse tree; live queries tag indices with callbacks and send filesystem events as files enter/leave the result set.[^src8]

**Performance** (vs. raw disk ~5.92 MB/s): BFS achieves 99% for sequential I/O; 50% overhead for metadata-intensive workloads due to indexing (BFS-noindex is roughly 2x faster on PostMark).[^src8]

**Vnode layer**: 57-function interface (`vnode_ops` struct); cookies provide per-file-descriptor state; `walk()` translates path components to vnids; `notify_listener()` powers the node monitor API.[^src8]
...
- [/software-engineering/sources/practical-file-system-design.md](/software-engineering/sources/practical-file-system-design.md) — BFS implementation detail: Giampaolo 1999
...
[^src8]: [Practical File System Design: The Be File System — Giampaolo 1999](sources/practical-file-system-design.md)
