...
sources:
  ...
  - path: raw/youtube/youtube-KuClyhvSzXk-every-frontend-system-design-pattern-w-senior-engineer-micro.md
    channel: youtube
    ingested_at: 2026-06-25
  - path: raw/web/web-designing-data-intensive-applications.md
    channel: web
...
## Designing Data-Intensive Applications (DDIA) — key framing

Martin Kleppmann's *Designing Data-Intensive Applications* (O'Reilly) is the canonical reference for engineers working on storage, streaming, and distributed data systems. Described by readers as "a mini-encyclopedia of modern data engineering."[^src5]

Key frameworks from DDIA:

**Reliability, scalability, maintainability** — the book opens by defining these terms precisely: what does "fault" vs. "failure" mean? How do you describe load and performance in a meaningful way? This conceptual foundation prevents teams from talking past each other.[^src5]

**Data models**: relational, document, and graph — each optimized for a different access pattern. Document models (denormalized) work when records are read as units; graph models work when relationships are the point; relational models work for structured joins and integrity.[^src5]

**Storage internals**: hash indexes, SSTables, LSM-trees, B-trees — understanding these tells you why write-heavy workloads favor LSM-trees (Cassandra, RocksDB) and read-heavy workloads favor B-trees (Postgres, MySQL).[^src5]

**Data encoding trade-offs**: XML/JSON (human-readable, verbose) vs. Avro/Thrift/Protocol Buffers (binary, compact, schema-enforced). Key consideration for pipeline evolvability.[^src5]

**Replication and consensus** (Part II): replication lag, consistency models, leader election algorithms (Paxos, Raft, ZAB/ZooKeeper). CAP theorem, version vectors (≠ vector clocks), Byzantine faults, causality and ordering.[^src5]

**Partitioning (sharding)**: range partitioning vs. hash partitioning; secondary indexes on partitioned data (scatter-gather vs. local indexes); re-balancing strategies.[^src5]

**Transactions and isolation levels**: fuzzy definitions of ACID, isolation levels from read-committed to serializable, and why the CAP theorem is "a bit of a red herring" in practice.[^src5]

**Batch and stream processing** (Part III): MapReduce model, modern frameworks (Spark), event stream processing, exactly-once semantics, and the challenges of "data in motion."[^src5]
...
[^src5]: raw/web/web-designing-data-intensive-applications.md — Goodreads community reviews of *Designing Data-Intensive Applications* by Martin Kleppmann (O'Reilly)
