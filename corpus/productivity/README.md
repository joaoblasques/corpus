---
type: hub
domain: productivity
status: draft
tags:
  - corpus/productivity
  - hub
created: 2026-06-12
updated: 2026-06-25
provisional: true
---

# Productivity

Personal and professional effectiveness for knowledge workers: mental models, time and focus management, note-taking / second-brain practice, working effectively with stakeholders, and learning how to learn. Provisional domain created in the 2026-06-12 email-backlog ingest (wave 3); 30-day review 2026-07-12.

## Pages

### Concepts
- [[productivity/mental-models|Mental Models]] — concept · draft · Circle of Competence, Thought Experiments, Map Is Not Territory, Ladder of Inference, Ishikawa Diagram; cross-links to Decision Making
- [[productivity/decision-making|Decision Making]] — concept · draft · rationality, ordinal vs cardinal utility, VNM expected utility, Bayesian beliefs, strategic interdependence, Keynesian beauty contest (MIT Game Theory Lecture 1)
- [[productivity/time-and-focus-management|Time & Focus Management]] — concept · draft · plan in systems, protect focus (time blocking, maker vs manager time, deep work), prioritize high-impact work; Mac personal-assistant automation (Shortcuts, Focus modes, dictation)
- [[productivity/learning-to-learn|Learning to Learn]] — concept · draft · senior growth without a mentor; writing-to-process; cross-training; environment as multiplier; practice like you play; ACTOR framework (Aim/Compress/Test/Own/Run)
- [[productivity/shipping-and-scope|Shipping and Scope]] — concept · draft · good-enough, "done is an agreement," perfectionism, post-launch contract
- [[productivity/working-with-stakeholders|Working with Stakeholders]] — concept · draft · stakeholder trust, relationship management, proactive alignment; human layer of technical work (psychological safety, empathy as data); idea-quality filter (4 gates)
- [[productivity/ai-augmented-knowledge-work|AI-Augmented Knowledge Work]] — concept · draft · standing context, voice files, loop engineering; three-brain model (human/AI/shared); information hierarchy (two-tier); two-layer second brain; cognitive uploading (NotebookLM/Steven Johnson); SuperWhisper context types
- [[productivity/obsidian-pkm|Obsidian & Personal Knowledge Management]] — concept · draft · local-first Markdown vault; linking over filing; emergent structure, MOCs, graph, canvas; context engineering + Kepano skills repo; VS Code alternative; Zettelkasten thinking/doing separation; Quartz SSG

### Entities
- [[productivity/quartz-ssg|Quartz SSG]] — entity · stub · TypeScript static-site generator for Obsidian vaults / digital gardens (jackyzha0/quartz, 12k+ stars)
