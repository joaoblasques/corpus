...
2. **Make data easy to use with the appropriate data model.** A bad model is painful to use and leads to messy data. The source teaches [Kimball dimensional modeling](/data-engineering/dimensional-modeling.md) — `dim_noun` dimension tables, `fct_verb` fact tables at the lowest grain, bridge tables for many-to-many, and **data marts with OBTs (One Big Table)** so metric calculations stay in one place and are consistent across teams [^src2]. The flow uses bronze/silver/gold layers to transform data progressively (see [medallion architecture](/data-engineering/medallion-architecture.md)) [^src2].
...
For an early-stage system, three signals alone provide huge value: **data freshness** (when was last successful load?), **volume checks** (records per hour), and **schema validation** (does incoming data match expected structure?) [^src4]. See [Data Observability](/data-engineering/data-observability.md) for the full observability pattern catalog.

## Code tests vs data-quality checks (different things, different timing)

A distinction worth keeping explicit: **code tests** and **data-quality checks** are not the same and run at different moments [^src5]. Code tests verify the *transformation logic is correct* and run **before code is deployed to production** (i.e. in CI, once per change — see [CI/CD for Data Infrastructure](/data-engineering/cicd-for-data-infrastructure.md)); data-quality checks verify the *data is correct* and run **every time the pipeline runs** (per-load, on live data) [^src5]. Both matter because *"ensuring your data is correct before stakeholders can access it is critical — imagine your pipeline accidentally adding (or removing) a 0 to a revenue metric"* [^src5]. The five-vertical data contracts above are the per-run checks; pytest-style unit tests on the pipeline code are the pre-deploy checks. (Practically, the pre-deploy gate is implemented with **pytest** — see [Pipeline Coding Patterns](/data-engineering/pipeline-coding-patterns.md) for the testing techniques.)

## Why the transform layer is a DQ surface

The transform step is itself a quality risk: "Each transform and SQL script is another opportunity to introduce errors," so **unit tests and data-quality checks "aren't just nice to have, they're essential for catching issues before they hit production"** [^src6]. This compounds with an **ownership** problem — transform tooling like [dbt](/data-engineering/dbt.md) has broadened who can build platform layers (engineers, analytics engineers, analysts), which is great for speed but blurs the line of responsibility: where does ownership of the core warehouse end and use-case-specific layers begin? Without clear boundaries, things get messy fast [^src6]. See [Data Transformation](/data-engineering/data-transformation.md) for the transform-layer challenges this DQ discipline guards against.
...
- [Dimensional modeling](/data-engineering/dimensional-modeling.md) — the modeling step in the 6-step framework.
- [Medallion architecture](/data-engineering/medallion-architecture.md) — quality rules are enforced at the silver layer.
- [Change data capture](/data-engineering/change-data-capture.md) — write disposition (`append` vs `merge`) is a recurring quality pitfall in change-driven loads.
- [dbt](/data-engineering/dbt.md) — lineage and SOT tracking referenced as the Source-of-Truth tooling.
- [Data Observability](/data-engineering/data-observability.md) — the instrumentation layer that detects when quality goals are not met; observability patterns (flow interruption, skew, lag, SLA misses).
