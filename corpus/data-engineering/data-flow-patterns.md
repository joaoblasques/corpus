...
> The two combine into the foundational rule of the page: **idempotency requires a replayable source *and* an overwritable sink** [^src1]. See [Idempotent Pipelines](/data-engineering/idempotent-pipelines.md).
...
For time-ranged pulls into an [SCD2](/data-engineering/scd2.md) table, **pause further pulls before rerunning a failed one** to avoid corrupting the table [^src1]. For full snapshots from a production table, **pull from a replica** [^src1].

## Behavioral patterns (how it handles failure)

- **Idempotent** — same inputs always produce the same output: reruns/backfills create no duplicates or schema changes. Requires a **replayable source and overwritable sink**. Pros: easy reruns/backfills, clean lineage, replayable debugging. Cons: longer dev time; hard to maintain idempotency when requirements change or a non-replayable enrichment source (an OLTP table) sneaks in [^src1]. This is the corpus's [Idempotent Pipelines](/data-engineering/idempotent-pipelines.md) concept, here framed as one *behavioral* choice among others.
...
- **Multi-hop** — keep data at separate levels of "cleanliness" with DQ checks between layers (the [medallion](/data-engineering/medallion-architecture.md) idea). Pros: rerun only failed transforms + dependents; intermediate tables pinpoint where a bug entered. Cons: storage + processing cost of copies; custom patterns have a steep learning curve — use well-established ones [^src1].
...
- [Idempotent Pipelines](/data-engineering/idempotent-pipelines.md) — the behavioral idempotent pattern in depth; replayable-source + overwritable-sink prerequisite
- [Data Ingestion Patterns](/data-engineering/data-ingestion-patterns.md) — the stream-vs-batch entry choice that precedes these patterns
- [Incremental Pipeline Design](/data-engineering/incremental-pipeline-design.md) — concrete extract/load/backfill mechanics for time-ranged pulls
- [Pipeline Coding Patterns](/data-engineering/pipeline-coding-patterns.md) — the *coding* (vs data-flow) patterns; part 2 of the same series
- [Medallion Architecture](/data-engineering/medallion-architecture.md) — the canonical multi-hop structure
- [Change Data Capture](/data-engineering/change-data-capture.md) — a replayable streaming source
