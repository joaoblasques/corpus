...
## Data Warehousing and OLAP (Ch. 4–5)

Data warehouses store integrated, historical data organized for analytics. The multidimensional **data cube** is the core structure — precomputed aggregates across all combinations of n dimensions. Key schema types: star (denormalized, fast), snowflake (normalized, slower joins), galaxy/fact-constellation (multiple fact tables sharing dimension tables) [^src6].

OLAP operations (roll-up, drill-down, slice, dice, pivot) enable interactive multidimensional analysis. Materialization strategy (full vs. partial vs. iceberg cube) is the main storage/performance trade-off.

Full treatment: [/data-engineering/olap.md](/data-engineering/olap.md)

## Frequent Pattern Mining (Ch. 6–7)

Frequent itemsets occur in ≥ min_sup% of transactions. The **Apriori algorithm** (Agrawal & Srikant, 1994) uses antimonotonicity (all subsets of a frequent itemset are frequent) to prune the search space. **FP-growth** encodes the database in a prefix tree and mines without candidate generation — generally faster for dense data [^src7].

Interestingness beyond support/confidence: lift, Kulczynski, cosine, and other null-invariant measures avoid misleading rules caused by highly frequent background patterns.

Advanced: constraint-based mining (antimonotonic, monotonic, succinct, convertible constraints), multilevel/multidimensional patterns, colossal patterns.

Full treatment: [/data-engineering/frequent-pattern-mining.md](/data-engineering/frequent-pattern-mining.md)

## Classification (Ch. 8–9)

**Classification** learns a model from labeled training tuples to predict class labels for new data. Major algorithms:

- **Decision trees**: recursive divide-and-conquer using attribute selection measures (information gain, gain ratio, Gini index). Post-pruning to prevent overfitting.
- **Naïve Bayes**: assumes class-conditional independence; fast, effective on text; Laplacian correction handles zero-probability cells.
- **SVM**: finds maximum marginal hyperplane; kernel trick handles non-linear boundaries.
- **Neural networks (backpropagation)**: multilayer perceptron; gradient descent weight update.
- **Ensemble methods**: bagging (bootstrap + vote), boosting (AdaBoost, iterative reweighting), random forests (bagging + random feature subsets).

Model evaluation: confusion matrix, accuracy, precision, recall, F1, ROC/AUC, k-fold cross-validation, paired t-test for model comparison.

Classification algorithms shared with statistical learning → also at [/ai-engineering/classification-methods.md](/ai-engineering/classification-methods.md).

## Cluster Analysis (Ch. 10–11)

**Clustering** groups objects by similarity without predefined labels (unsupervised). Core method families:

- **Partition-based** (k-means): assign to nearest centroid, recompute, repeat. Sensitive to initialization; k-Means++ improves seeding. Fails on non-convex clusters.
- **Hierarchical** (agglomerative/divisive): build dendrogram using single-link, complete-link, average-link, or centroid linkage.
- **Density-based** (DBSCAN): core points, border points, noise; finds arbitrary-shape clusters; requires ε and MinPts.
- **Probabilistic** (GMM + EM): soft assignments; generalizes k-means to non-spherical clusters.
- **Advanced** (spectral, subspace, biclustering): handle high-dimensional data and non-convex cluster shapes.

Clustering algorithms shared with statistical learning → also at [/ai-engineering/clustering-methods.md](/ai-engineering/clustering-methods.md).

## Outlier Detection (Ch. 12)

Three families: **statistical** (model distribution, flag low-probability objects; kernel density estimation), **proximity-based** (distance-based and LOF — Local Outlier Factor — which compares local densities), **clustering-based** (small or sparse clusters).

High-dimensional challenge: standard distances converge → use rank-based methods (HilOut) or subspace-specific detection.
