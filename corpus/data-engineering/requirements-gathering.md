...
- **Semantic understanding** — what does the data represent, and which business process generates it? (Informs modeling — see [Meaning in Data Modeling](/data-engineering/data-modeling-meaning.md).)
...
- **Business-rule checks (QA)** — which DQ metrics matter; which numeric fields can't diverge by more than *x%* across loads? (Feeds [data contracts](/data-engineering/data-quality.md).)
...
A complementary framing for *greenfield or upgrade* architecture work: start from the **outputs the business needs and work backward**, rather than forward from all the available data sources [^src2]. The pitch is that "Output Led Engineering" both sets sound foundations from the start *and* **narrows scope** so the project doesn't become overwhelming — while still delivering results to the business fast [^src2]. This is the same instinct as step 2's *business-impact* question and the [data-engineer value](/data-engineering/data-engineer-role.md) pillar of "know how the company makes money" — requirements are derived from the desired output, not the other way around.

## Related

- [The Data Engineer Role](/data-engineering/data-engineer-role.md) — business impact as the first pillar of DE value; prioritization-as-leverage
- [Data Quality](/data-engineering/data-quality.md) — the business-rule (QA) questions become data contracts validated before use
- [Meaning in Data Modeling](/data-engineering/data-modeling-meaning.md) — the semantic-understanding question feeds modeling
- [DE Portfolio Projects](/data-engineering/de-portfolio-projects.md) — "think backward from what the business needs" is the recommended starting exercise
- [Small-Scale Pipeline Design](/data-engineering/small-scale-pipeline-design.md) — applies this "understand the problem scope" discipline to the everyday small pipeline (its five-question scoping step is a lighter version of these five steps)
- [Working with Stakeholders](/productivity/working-with-stakeholders.md) — the relationship side of sign-off and buy-in (productivity)
