...
## Four-tier memory hierarchy (ByteByteGo)

A more granular model distinguishes four tiers by persistence and mechanism [^src37]:

| Tier | What it holds | Persistence |
|---|---|---|
| **Context window** | Active conversation, current task state | Current session; lost on reset |
| **Short-term / session store** | Cross-turn summaries, working hypotheses | Session duration; may survive compaction |
| **Long-term store** | Persistent facts, preferences, past decisions (vector DB or graph) | Indefinite; retrieved on demand |
| **Cold archive** | Conversation logs, raw trace data | Indefinite; rarely retrieved; bulk storage |

The core challenge is **retrieval** — not storage. At long-term store scale, a naive similarity search produces "a heap of vaguely relevant memories with no sense of which is most applicable." Structured retrieval (scope-filtered hybrid search, temporal graphs) determines memory quality far more than storage choice. [^src37]

**Four functional types** (orthogonal to the tier hierarchy) [^src37]:
- *Working memory* — immediate task variables, current tool outputs; maps to context window
- *Episodic memory* — past interaction summaries, specific conversation history ("last Tuesday's call")
- *Semantic memory* — general knowledge, stable facts, domain understanding
- *Procedural memory* — learned behaviors, tool-use patterns, "how to do X" workflows

**Key tradeoffs** [^src37]:
- Recency bias vs relevance: recent memories aren't always most useful; retrieval must balance recency and semantic match
- Summarization vs fidelity: compressing older memories saves tokens but loses detail; compression timing and strategy matter
- Staleness: facts change; memory systems need invalidation paths (see Typed memory architecture above)
- Memory poisoning: adversarial or erroneous inputs that permanently corrupt long-term memory if write gates are too permissive
...
[^src37]: [How AI Agents Manage Memory and Avoid Forgetfulness](../../raw/web/web-how-ai-agents-manage-memory-and-avoid-forgetfulness-a914ab49.md) — ByteByteGo, 2026-08-21; four-tier hierarchy, functional memory types, retrieval as the core hard problem
