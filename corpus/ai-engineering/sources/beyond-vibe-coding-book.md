...
**Ch2 — The art of the prompt.** "In vibe coding, prompts are the new source code." [^ch2] The full technique toolkit: zero-shot, one-shot, few-shot, Chain-of-Thought (CoT), role prompting, contextual prompting, metaprompting, self-consistency, ReAct. Context window defined as the maximum tokens for a single interaction. Antipatterns catalogued: vague prompt, overloaded prompt, missing success criteria, ignoring AI clarification, inconsistency, vague references. See [[ai-engineering/prompt-engineering|Prompt Engineering]].
