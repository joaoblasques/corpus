...
- **~5 parallel agents, all the time.** "At the moment I have like five agents running" during the recording; "I always have a bunch of agents running" [^src1]. Anthropic calls running many sessions at once **"multi-clouding"** [^src1]. Surface split: roughly a third terminal, a third desktop app, a third the iOS app — "a third of my code... is the iOS app, which is just so surprising" [^src1]. See [Claude Code](/ai-engineering/claude-code.md), [Multi-Agent Systems](/ai-engineering/multi-agent-systems.md).
- **Plan mode first.** "I start almost all of my tasks in plan mode, maybe like 80%." Plan mode is "actually really simple... we inject one sentence into the model's prompt to say, 'Please don't write any code yet.'" After a good plan he auto-accepts edits: "if the plan works good, it's just going to one-shot it" [^src1]. Echoes the explore→plan→code pattern in [Agentic Coding](/ai-engineering/agentic-coding.md).
- **Use the most capable model.** Tip #1: "use the most capable model... I have maximum effort enabled always." Counterintuitively a cheaper model can cost *more*: "because it's less intelligent, it actually takes more tokens in the end to do the same task" [^src1]. Connects to [Agent Cost Management](/ai-engineering/agent-cost-management.md).
- **Run times are growing.** Sonnet 3.5 a year ago "could run for maybe 15 or 30 seconds before... going off the rails"; Opus 4.6 runs "10, 30, 20, 30 minutes unattended," with examples running "hours or even days" and "some examples where they ran for many weeks" [^src1]. See [Long-Running Agents](/ai-engineering/long-running-agents.md).
...
- **The role shift.** "The title software engineer is going to start to go away... replaced by builder," or "everyone's going to be a product manager and everyone codes." On the Claude Code team "everyone codes" — PM, EM, designer, finance, data scientist; roles have "maybe a 50% overlap" [^src1]. The most-rewarded people will be "curious... generalists [who] cross over multiple disciplines" [^src1]. Career framing lives in [AI and the Job Market](/ai-business/ai-and-the-job-market.md).
...
Anthropic's product ladder is "coding, then tool use, then computer use" — chosen because each step is how they study safety in three layers: **mechanistic interpretability** (Chris Olah; monitoring neurons, superposition), **evals** (the model "in a petri dish"), and **behavior in the wild** [^src1]. Claude Code was used internally ~4–5 months before release to study safety, shipped as a "research preview"; the **open-source sandbox** ("works with any agent, not just Claude Code") is Anthropic's "race to the top" lever [^src1]. See [Agent Security](/ai-engineering/agent-security.md).

## Contradictions / tensions to track

- **Scaffolding stance vs. the corpus's harness emphasis.** Cherny argues "you don't really need" the scaffolding now — "don't box the model in," bet on the general model, expect the next model to wipe out a 10–20% scaffolding gain [^src1]. The corpus's [Agent Harness](/ai-engineering/agent-harness.md) / [Agentic Coding](/ai-engineering/agentic-coding.md) pages (Builder.io AX, Osmani) hold that the *harness* — context, hooks, tests, review loops — matters as much as the model. These are not flatly opposed (Cherny still relies on plan mode, sandboxing, CLAUDE.md, `/goal`, and human review), but they weight model-vs-scaffolding differently; the reconciliation is *minimal but well-designed* scaffolding around a strong general model, not zero scaffolding.

## See also

- [Claude Code](/ai-engineering/claude-code.md) — the tool; parallel sessions, plan mode, model config
- [Agentic Coding](/ai-engineering/agentic-coding.md) — orchestration theory this interview instantiates
- [Vibe Coding](/ai-engineering/vibe-coding.md) — "coding is describing, not writing"
- [Long-Running Agents](/ai-engineering/long-running-agents.md) — multi-hour/day runs
- [AI Product Management](/ai-engineering/ai-product-management.md) — latent demand, "everyone's a PM"
- [Agent Cost Management](/ai-engineering/agent-cost-management.md) — loose-with-tokens, best-model economics
