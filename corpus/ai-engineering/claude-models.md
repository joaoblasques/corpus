...
Released ~June 9 2026 and available across all Anthropic surfaces — claude.ai, Claude Code (web + CLI), and Cowork [^src6]. It "works more like a seasoned engineer: designed to investigate your codebase before it acts… and check its work before reporting it's done", excelling at long-running, asynchronous, multi-hour-to-multi-day sessions [^src5]. It is **not** the default model; select it with `/model fable`, ideally paired with `/goal` so it works until a completion condition is met [^src5]. Anthropic's developer marketing positions Fable 5 as leading **SWE-Bench Pro at 80.3%** [^src22] — above the 69.2% claimed for Opus 4.8 at its launch, consistent with Fable 5 sitting above Opus in the lineup. The same `claude.com/solutions/coding` page advertises customer outcomes of "60× faster" code-review feedback and a "95%" reduction in test-execution time [^src22]; these are vendor marketing claims, not independent benchmarks.
...
[^src22]: [Programmierung | Claude](../../raw/web/web-programmierung-claude-bbf1e585.md) — Anthropic marketing page (`claude.com/solutions/coding`); also collected in Japanese ([コーディング](../../raw/web/web-claude-469a16cf.md)) as identical content.
