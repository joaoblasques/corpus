...
## Prompt engineering in the context of the Compute Schism (Fleuret, 2024)

Fleuret's framing places prompt engineering as the first and simplest technique in the "Compute Schism" toolkit — adapting a large pre-trained model to a task without retraining [^src24]:

**Core principle**: Move information traditionally encoded in model parameters into the input token sequence. Prompt engineering is bounded by the model's **context size** (max token count), which in turn is bounded by the quadratic cost of attention (see [Attention Mechanisms](/ai-engineering/attention-mechanisms.md) § computational cost).[^src24]

**Few-shot prediction** (§ 7.1 in LBDL): An LLM can perform classification without fine-tuning by including examples in the prompt. GPT models demonstrated this capability; it generalizes to any task with clear input-output structure.[^src24]

**Chain-of-thought prompting** [Wei et al., 2022; Kojima et al., 2022]: Including intermediate steps in the prompt biases the model to generate step-by-step reasoning before producing the final answer. Each step is simpler and more deterministic than the full task — hence better modeled during training and more reliably predicted. "Let's proceed step by step" can be added as a suffix to elicit this behavior even without demonstration examples [^src24].

**Expert persona prompting** [Xu et al., 2023]: Stating in the prompt that the response is generated by a skilled professional ("You are an expert in X") improves response quality on average, because such framings correlate with high-quality outputs in the training corpus.[^src24]

**RAG as prompt engineering** (§ 8.1 in LBDL): Retrieval-Augmented Generation (see [RAG](/ai-engineering/rag.md)) is a form of prompt engineering — retrieved documents are injected into the prompt to extend the model's effective knowledge beyond its training cutoff.[^src24]
...
[^src24]: raw/_inbox/pdf-the-little-book-of-deep-learning-part-03.md — Fleuret, § 8.1
