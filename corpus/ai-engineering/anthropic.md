...
**TL;DR.** Anthropic PBC is the AI lab behind the Claude model family and the [[ai-engineering/claude-code|Claude Code]] / [[ai-engineering/claude-cowork|Claude Cowork]] products. As of mid-2026 it reportedly closed a ~$65B round at a ~$965B valuation, passing OpenAI for the first time [^src4]. The current Claude lineup spans Haiku (fast/cheap) → Sonnet → Opus → Fable/Mythos (frontier) [^src9]. This page tracks the lineup; product specifics live on the Claude Code and Cowork pages.
