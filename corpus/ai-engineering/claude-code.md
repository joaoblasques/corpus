...
sources:
  ...
  - path: raw/web/web-the-difference-between-claude-code-and-cowork-forte-labs-9dd81bc1.md
    channel: web
    ingested_at: 2026-08-14
  - path: raw/web/web-on-ai-coding-and-its-discontents-cal-newport-f18f4c7c.md
    channel: web
    ingested_at: 2026-08-14
aliases:
  - Claude Code
...
[^src60]: [The Difference Between Claude Code and Cowork](../../raw/web/web-the-difference-between-claude-code-and-cowork-forte-labs-9dd81bc1.md) — Forte Labs, 2026-06-29

## Claude Code vs Anthropic Cowork

Anthropic now ships two tools that both control a web browser and access local files, with different tradeoffs[^src60]:

| | Claude Code | Cowork |
|---|---|---|
| **Setup** | Install locally, run from terminal (or web version) | Lives inside Claude desktop app — no setup |
| **Target user** | Developers; power users comfortable with CLI | Non-developers; zero configuration |
| **Token efficiency** | Higher — processes files natively | Lower — extra overhead from screenshots, image processing |
| **Real-time control** | Shows every step; can redirect immediately | Wait for completion before redirecting |
| **When to use** | Performance-sensitive, large codebases, power workflows | Ease-of-use priority; first foray into agent tools |

Forte's take: "If you're in doubt about either ease of use or security, start with Cowork. You can always advance from there to Code later on."[^src60]

Reason Cowork exists: Claude Code's setup was intimidating for non-developers. Cowork trades token efficiency for zero-friction entry.[^src60]

## Critique: when agentic coding goes wrong

Cal Newport's 2026 field report from 300+ software engineers[^src61]:

- A senior engineer who described Claude Code as overnight changing their job ("tasks that took a week now take 2 days") sent a follow-up: code generated by Claude Code crashed their product twice; one more incident and he'd be fired.
- Root cause: AI-generated code looks reasonable but can contain hard-to-spot bugs. Reviewing code you didn't write is "famously hard" — the tendency is to skip the review step ("easy to just blow it off, especially when we are all trying to 10x our velocity").
- Result: the engineer returned to largely programming by hand, using LLMs only for "narrow or particularly annoying tasks (say like writing tests or throw-away scripts)."
- Newport: coding harnesses are useful but "encourage laziness" and create a "mind-numbing workflow." Junior developers relying entirely on them never acquire expertise to become senior developers capable of designing complex systems.
- Token economics: as frontier labs reduce compute subsidies, burning through tokens for workable results is becoming prohibitively expensive.

Newport's broader point: even in the best-case-scenario setting for AI (code and math — structured languages + massive training data), "we're still struggling to figure out how to actually use these tools in a way that makes sense in the long run." AI is a normal technology, not an infinity machine.

[^src61]: [On AI Coding and Its Discontents](../../raw/web/web-on-ai-coding-and-its-discontents-cal-newport-f18f4c7c.md) — Cal Newport, 2026-08-11
