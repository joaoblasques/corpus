...
- **Multimodal peers.** OpenAI is named as a peer point of comparison for unified multimodal models: **Black Forest Labs' FLUX 3** — a unified model spanning **image, video, audio, and action prediction** with robotics applications (including a **FLUX-mimic** video-action robotics instantiation) — is reported to outperform existing models across tasks, its title claiming wins over **Seedance 2.0, Gemini Omni, and Grok Imagine** [^src11]. The model is available for early access [^src11]. The framing places OpenAI's model line inside a widening multimodal/embodied competitive field rather than a purely text-and-code one.
