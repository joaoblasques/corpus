...
Some modern frontier models replace the dense [Transformer](/ai-engineering/transformer.md) FFN with MoE. Instead of one feed-forward network per layer, the model has many parallel FFNs (the **experts**) and a tiny **router network** that picks which experts process each token [^src1].
...
See [vLLM](/ai-engineering/vllm.md) for the fuller Elastic EP write-up (scale-up/scale-down flow detail, cross-rank barrier coordination) and [vLLM Semantic Router](/ai-engineering/vllm-semantic-router.md) for routing decisions that sit in front of MoE serving capacity like this.

## See also

- [Transformer](/ai-engineering/transformer.md) — MoE replaces the dense feed-forward network inside transformer layers
- [LLM](/ai-engineering/llm.md) — MoE is a scaling strategy for the parameter count of modern LLMs
- [Claude Model Lineup](/ai-engineering/claude-models.md) — frontier model families where dense-vs-MoE is a live architectural choice
- [vLLM](/ai-engineering/vllm.md) — serving engine with expert-parallel, Elastic EP runtime scaling, and quantized MoE execution backends
- [MiniMax M3](/ai-engineering/minimax-m3.md) — MXFP8-quantized MoE model with day-0 vLLM support
- [vime](/ai-engineering/vime.md) — RL post-training framework with MoE-specific train-inference alignment (R3 routing replay)
